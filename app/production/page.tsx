...
import { ArrowLeft, Calendar, Loader2, Factory } from "lucide-react"
...
// --- BUSINESS LOGIC ---
// Сводка по дням/товарам: выпуск vs реальные операции (продажи/бонусы/обмены/возвраты)
...
const calculateProductionSummary = (
  ...
  // 1) Собираем выпуск по датам/товарам
  ...
  // 2) Подтягиваем движения (продажи/бонусы/обмены/возвраты) по этим же датам/товарам
  movements.forEach((m) => {
  ...
  // 3) Чистый расход и теоретический остаток
...
function useProductionData() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (!prodRes.ok || !batchRes.ok || !movRes.ok)
...
// --- FORM: добавление партии ---
// Бонусный фонд из формы убрали, считаем только произведённое количество.
...
const AddBatchForm = ({
  ...
  products: Product[]
  onSave: (data: { date: string; productId: string; producedQty: number; comment: string | null }) => Promise<void>
}) => {
    ...
    <Card className="border-zinc-800 bg-zinc-900/30">
      <CardHeader className="border-b border-zinc-800/50 pb-4">
        ...
        <CardDescription className="text-zinc-400">
          Фиксация выпуска продукции за смену
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="flex justify-end pt-2">
          <Button
            ...
            disabled={isSubmitting || !date || !productId || !producedQty}
...
const SummaryTable = ({ rows }: { rows: ProductionSummaryRow[] }) => (
  <Card className="border-zinc-800 bg-zinc-900/30">
    <CardHeader className="border-b border-zinc-800/50 pb-4">
      ...
      <CardDescription className="text-zinc-400">
        Сверка выпуска с фактическими продажами день-в-день
      </CardDescription>
    </CardHeader>
        ...
        <Table>
          <TableHeader className="bg-zinc-900/80 sticky top-0 z-10 backdrop-blur-sm">
            <TableRow className="border-zinc-800 hover:bg-transparent">
              ...
              <TableHead className="text-right text-zinc-200 font-semibold border-l border-zinc-800/50">
                Расход (чистый)
              </TableHead>
              <TableHead className="text-right font-bold border-l border-zinc-800/50">
                Остаток (теор.)
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                ...
                >
                  ...
                  <TableCell
                    className={`text-right font-bold border-l border-zinc-800/50 bg-zinc-900/20 ${
                      row.theoreticalRestQty < 0
                        ? "text-red-500"
                        : "text-green-500"
                    }`}
                  >
...
export default function ProductionPage() {
  const { products, batches, movements, loading, error, setBatches } =
    useProductionData()
    ...
    <main className="min-h-screen p-4 md:p-6 lg:p-8 bg-zinc-950 text-zinc-100">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              Учёт партий и сверка с продажами
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <div className="flex flex-wrap items-end gap-4 bg-zinc-900/30 p-4 rounded-lg border border-zinc-800">
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2 text-zinc-400">
              <Calendar className="h-4 w-4" /> По дату
            </Label>
            <Input
              type="date"
              value={dateTo}
