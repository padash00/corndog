          ...
          <TableHeader className="bg-zinc-900/80 sticky top-0 z-10 backdrop-blur-sm">
            <TableRow className="border-zinc-800 hover:bg-transparent">
              ...
              <TableHead className="text-zinc-400 min-w-[200px]">
                {showSubLabel ? "Магазин" : "Название"}
              </TableHead>
              <TableHead className="text-right text-zinc-300">
                Выручка
              </TableHead>
              <TableHead className="text-right text-green-400 font-bold">
                Прибыль
              </TableHead>
              ...
              <TableHead className="text-right text-zinc-500 text-xs w-24">
                Доля проблем
              </TableHead>
              <TableHead className="text-right text-zinc-500 text-xs w-24">
                Доля бонусов
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                ...
                >
                  ...
                  <TableCell className="text-right text-zinc-300 whitespace-nowrap">
                    {row.revenue.toLocaleString("ru-RU")} ₸
                  </TableCell>
                  <TableCell className="text-right text-green-400 font-bold whitespace-nowrap">
...
export default function ReportsPage() {
  ...
  const periodLabel =
    dateFrom && dateTo
      ? `${format(new Date(dateFrom), "dd.MM.yyyy", { locale: ru })} — ${format(
          new Date(dateTo),
          "dd.MM.yyyy",
          { locale: ru }
        )}`
    ...
    <main className="min-h-screen p-4 md:p-6 lg:p-8 bg-zinc-950 text-zinc-100">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="space-y-8">
          <ReportTable
            title={`📍 Сводка по районам (${periodLabel})`}
            rows={districtRows}
          />
          <ReportTable
            title={`🏪 Сводка по магазинам (${periodLabel})`}
            rows={storeRows}
            showSubLabel
          />
        </div>
